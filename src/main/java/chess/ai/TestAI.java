...
public class TestAI implements Player {
    ...
    */
     ...
     */
    public TestAI(GameController controller, ChessColor ownColor, AIOptions 
            aiOptions) {
    ...
    private int evaluateBoard(ChessColor colorToMove) {
        ...
        int value;
        if(ownColor == WHITE)
            value = pieceValueWhite(ownPieces) - pieceValueBlack(enemyPieces);
        else 
            value = pieceValueBlack(ownPieces) - pieceValueWhite(enemyPieces) ;
     ...
     */
    private int pieceValueWhite(ArrayList<Piece> pieces) {
        ...
        for (Piece piece : pieces) {
            ...
            switch (piece.getType()) {
                ...
                case PAWN:
                    ...
                    int file = coord.getY();
                    /* doubled pawns */
                    if (board.getPawnStructWhite(file) > 1) {
                        value -= PAWN_DOUBLE_MALUS;
                    }
                    /* isolated pawn */
                    if (board.getPawnStructWhite(file - 1) == 0
                            && board.getPawnStructWhite(file + 1) == 0) {
                        value -= PAWN_ISOLATED_MALUS;
                    }
                    /* pawn position */
                        value += WHITE_PAWN_BONUS_MATRIX[coord.getX()][file];
                    break;

                    
                case QUEEN:
                    /* distance to enemy king, weighted with 0.5 */
                    value += QUEEN_BASIC_VALUE - 0.5 * (coord
                                    .distance(board.getBlackKingCoord()));
                ...
                case BISHOP:
                    ...
                    for (Coordinate auxCoord : coord.getDiagCoordFront(WHITE)) {
                        if (board.getPieceTypeOnCoord(auxCoord) == PAWN) {
                            value -= BISHOP_DIAG_PAWN_MALUS;
                        }
                    }
                    
                    value += WHITE_BISHOP_BONUS_MATRIX[coord.getX()][coord.getY()];
                ...
                case KNIGHT:
                    ...
                    value += KNIGHT_BASIC_VALUE + KNIGHT_BONUS_MATRIX[coord.getX()][coord.getY()];
                    break;
                ...
                case ROOK:
                    ...
                    /* open file */
                    if (board.getPawnStructWhite(rookFile) == 0) {
                        if (board.getPawnStructBlack(rookFile) == 0) {
                    ...
                    /* rook position */
                    value += WHITE_ROOK_BONUS_MATRIX[coord.getX()][rookFile];
                ...
                case KING:
                    ...
                    if (gameStage == ENDGAME) {
                        value += KING_BONUS_MATRIX_ENDGAME[coord.getX()]
                                                                   [coord.getY()];
                    } else {
                           value += WHITE_KING_BONUS_MATRIX_MIDDLEGAME[coord.getX()]
                                                                   [coord.getY()];
                    }
            }
        }
...
/**
     ...
     * @return          sum of piece values  
     */
    private int pieceValueBlack(ArrayList<Piece> pieces) {
        ...
        for (Piece piece : pieces) {
            ...
            switch (piece.getType()) {
                ...
                case PAWN:
                    ...
                    int file = coord.getY();
                    /* doubled pawns */
                    if (board.getPawnStructBlack(file) > 1) {
                        value -= PAWN_DOUBLE_MALUS;
                    }
                    /* isolated pawn */
                    if (board.getPawnStructBlack(file - 1) == 0
                            && board.getPawnStructBlack(file + 1) == 0) {
                        value -= PAWN_ISOLATED_MALUS;
                    }
                    /* pawn position */
                        value += BLACK_PAWN_BONUS_MATRIX[coord.getX()][file];
                ...
                case QUEEN:
                    /* distance to enemy king, weighted with 0.5 */
                    value += QUEEN_BASIC_VALUE - 0.5 * (coord
                                    .distance(board.getWhiteKingCoord()));
                    //value += queenBonus(piece);
                    break;
                ...
                case BISHOP:
                    ...
                    /* is the bishop blocked by pawns in front of him? */
                    for (Coordinate auxCoord : coord.getDiagCoordFront(BLACK)) {
                        if (board.getPieceTypeOnCoord(auxCoord) == PAWN) {
                            value -= BISHOP_DIAG_PAWN_MALUS;
                        }
                    }

                    value += BLACK_BISHOP_BONUS_MATRIX[coord.getX()][coord.getY()];          
                ...
                case ROOK:
                    ...
                    /* rook position */
                    value += BLACK_ROOK_BONUS_MATRIX[coord.getX()][rookFile];
                ...
                case KING:
                    ...
                    if (gameStage == ENDGAME) {
                        ...
                        value += BLACK_KING_BONUS_MATRIX_MIDDLEGAME[coord.getX()]
                                                                   [coord.getY()];
                    }
            }
        }
    ...
    private int bishopBonus(Coordinate coord, ChessColor color) {
        ...
        /* is the bishop blocked by pawns in front of him? */
        for (Coordinate auxCoord : coord.getDiagCoordFront(color)) {
            if (board.getPieceTypeOnCoord(auxCoord) == PAWN) {
                bonus -= BISHOP_DIAG_PAWN_MALUS;
            }
        }
        if (color == BLACK) {
            bonus += BLACK_BISHOP_BONUS_MATRIX[coord.getX()][coord.getY()];
        } else {
            bonus += WHITE_BISHOP_BONUS_MATRIX[coord.getX()][coord.getY()];
        }
        return bonus;
    }
     ...
     * function. 
     * 
     * @param coord     coordinate of the pawn that is evaluated 
     * @param color     color of the pawn that is evaluated
     * @return          value of static evaluation function
     */    
    private int pawnBonus(Coordinate coord, ChessColor color) {
        
        int bonus = 0;
        int file = coord.getY();
        /* doubled pawns */
        if (board.getPawnStruct(color, file) > 1) {
            bonus -= PAWN_DOUBLE_MALUS;
        }
        /* isolated pawn */
        if (board.getPawnStruct(color, file - 1) == 0
                && board.getPawnStruct(color, file + 1) == 0) {
            bonus -= PAWN_ISOLATED_MALUS;
        }
        /* pawn position */
        if (color == BLACK) {
            bonus += BLACK_PAWN_BONUS_MATRIX[coord.getX()][file];
        } else {
            bonus += WHITE_PAWN_BONUS_MATRIX[coord.getX()][file];
        }
     ...
     * function. 
     * 
     * @param coord   coordinate of piece to be evaluated
     * @param color   color of piece to be evaluated
     * @return        value of static evaluation function
     */    
    private int rookBonus(Coordinate coord, ChessColor color) {
        
        int bonus = 0;
        int file = coord.getY();

        /* open file */
        if (board.getPawnStruct(color, file) == 0) {
            if (board.getPawnStruct(color.getInverse(), file) == 0) {
        ...
        /* rook position */
        if (color == BLACK) {
            bonus += BLACK_ROOK_BONUS_MATRIX[coord.getX()][file];
        } else {
            bonus += WHITE_ROOK_BONUS_MATRIX[coord.getX()][file];
        }
        return bonus;
    }
     ...
     * @return          value of static evaluation function
     */
    private int kingBonus(Coordinate coord, ChessColor color) {
        
        /* different matrices are used depending on stage of game */
        if (gameStage == ENDGAME) {
            return KING_BONUS_MATRIX_ENDGAME[coord.getX()]
                                                       [coord.getY()];
        } else {
            if (color == BLACK) {
               return BLACK_KING_BONUS_MATRIX_MIDDLEGAME[coord.getX()]
                                                       [coord.getY()];
            } else {
               return WHITE_KING_BONUS_MATRIX_MIDDLEGAME[coord.getX()]
                                                       [coord.getY()];
            }
        }
    }
